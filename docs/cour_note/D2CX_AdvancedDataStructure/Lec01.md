...
- 当我们访问到某个点 X 时：
    ...
    - 如果 P 不是根节点：
        ...
        - 当情况为 LL / RR 时，进行两次 [Single Rotation](#ll--rr)，我们称之为 zig-zig；
...
所谓的摊还分析，分析的是摊还边界，或者说瘫痪时间复杂度。平常我们分析时间复杂度时，针对的往往是某个具体的操作。而摊还分析的对象则是一个数据结构的一系列操作，而这一系列操作中有成本较低的，也有成本较高的，而且操作之间也可能有互相影响。
