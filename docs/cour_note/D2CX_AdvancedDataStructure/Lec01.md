...
我们知道，对于一棵二叉搜索树，其对点的操作代价为 $O(\log n)$。然而在最坏情况下，它会退化成 $O(n)$，例如这是一棵只有左子树的链型二叉树，那么操作这里唯一的叶子节点就是 $O(n)$。
...
!!! definition "AVL Tree"
    ...
    !!! definition "Balance Factor"
        一个节点的**平衡因子(Balance Factor)BF**用来描述一个节点的平衡状态，对于节点 $T_p$，它的左子树为 $T_l$，右子树为 $T_r$，则：
...
!!! proof "Height of AVL Trees"
    ...
    我们记 $F_h$ 是高度为 $h-1$ 的 AVL 树所包含的最少节点数，则有如下递推关系：
...
而我们谈论 AVL 树时，我们希望以一个动态的视角来谈论 AVL 树，即讨论：某一棵树是否是 AVL 树、在某个操作后是否仍然是 AVL 树、如何将它重新变为 AVL 树。重点在于，讨论最后一个问题，即“重新变为 AVL 树”这个问题时，我们默认这棵树在一次其他操作之前是一棵 AVL 树。
...
首先，我们遇到的问题是什么？是 Trouble Maker 进来以后，Trouble Finder 发现 $h(T_l) - h(T_r) = 2$了，用通俗的话来说就是左子树太高了，然而在这种情况下，左子树依然还站在一个台子（Trouble Finder）上和 Trouble Finder 比身高，所以我们需要将它从台子上请下来。也就是说，既然左子树太高了，那么我们就不**希望**让它再成为谁的子树，因为成为谁的子树必然意味着这条路径会更长。
...
- 当我们访问到某个点 X 时：
    ...
    - 如果 P 不是根节点：
        ...
        - 当情况为 LL / RR 时，进行两次 [Single Rotation](#ll--rr)，使得 X、P、G 的顺序逆转，像跷跷板一样，我们称之为 zig-zig；
...
虽然摊还分析第一次是在第一节课被提到的，但是由于之后频繁出现，而且摊还分析也是一项重要的技能，所以我单独开了一篇文章来讲摊还分析。