...
!!! tip "字扩展：$256\text{K}\times 8$ RAM"
    ...
    - 输入信号和 0-15 位地址通过总线传递给每个 $256\text{K}\times 8$ RAM，输出信号通过总线从每个 $256\text{K}\times 8$ RAM 中通过三态门接出，可以理解为 RAM 的“**并联**”；
    - 通过 16-17 位地址来选择访问哪个 $256\text{K}\times 8$ RAM，再根据 0-15 位地址来选择访问该 RAM 中的具体哪个 word；
