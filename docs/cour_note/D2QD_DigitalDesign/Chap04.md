...
> 或者从 Clocked Latch 本身的角度考虑，其写入操作是非原子的。具体对 SR 触发器而言，在 C 置 `1` 时 S 和 R 上的任何操作都能够对这个 Latch 进行即时写入。因此，如果在这期间输入端出现了毛刺，触发器也会相应地捕获毛刺并写入异常数据，也就是产生诸如 `1s-catching` 的这类现象。

> 进一步而言，这种非原子性是一种 inherent 的性质，无法真正 eliminate 掉，而 D Latch 的优势在于两点：1）写入结果的原子化，即写入的结果始终只由 D 决定，并且是 explicit assigned 的；2）写入过程的“准原子化”：通过收缩写入过程的时间窗口，将其严格限制在 edge 附近，有效地在时间上近似了原子操作。这样的设计使得绝大多数在时钟边沿之外出现的毛刺被有效排除，只有极少数恰好在采样窗口内对数据造成干扰的毛刺才可能导致异常写入。因此，由毛刺导致的异常写入在统计意义上被显著减少。这种技术虽不能绝对杜绝所有毛刺问题（如精确落在采样窗内的毛刺），但在实际工程中已能充分满足系统对稳定性的要求。
