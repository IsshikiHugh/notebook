...
> 事实上，除 1s-catching 的情况之外，也同样存在 0s-catching 的情况。这里我们将这两种情况统称为 "Glitch Catching" 并从 SR 触发器的工作机制展开讨论。

> 为分析工作机制，我们引入“瞬时”和“持续”两个概念，其含义是显然的。在此基础上，不难发现 SR 触发器的两个保持态（C 置 `0` 保持；C 置 `1`，S 和 R 置 `0`）均为持续状态，而写入操作（S 置 `1` 或 R 置 `1`，并将数据写入到主锁存器中）为瞬时状态。

> 而当我们使用第二个保持态时，从实际工作原理上而言，并非是使用了“保持”这一操作，而是使用了“允许写入但不写入”。这种“允许写入”的状态是持续状态，因此可能会受到出现噪音导致的额外写入操作影响，从而导致数据被改写。在实际使用中，我们不应该使用这种不严谨的状态来作为实际工作状态。

> 进而，我们可以引出边沿触发式 D 触发器，其新增的两个设计：1)边沿触发，从而尽可能缩短写入的持续时间，消除了“允许写入”的可能性；2) D 输入信号，持续写入一个确定的信号值，消除了“不写入”的可能性。能够有效地解决噪音信号导致误写入的问题。

> 需要指出的是，从理论分析的角度，毛刺对于触发器的影响是无法规避的；但是从统计意义而言，使用边沿触发机制可能导致的误差是可以接受的。读者可以参阅[这份英文的讨论帖](https://yardrat0117.github.io/FlipFlopDiscussion.html)。
...
<!-- 这个图应该是没错的，注意分析 OC 时补上 default 情况。# MinJoker
