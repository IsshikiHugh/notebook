...
!!! note "Generic State Diagram Template"
    ...
    ???+ note "模版图的直接解释"
        ...
        > 可以对照缩写对照表和结合下方这个状态机的 🌰 来看，主要掠过细节解释模版图片中的要素是什么。

        - $S_O$ 和 $S_D$ 是两个状态的符号；
        - $S_O \rightarrow S_D$ 有一条有向边，表示当转移条件 TC 为 `1` 时，状态 $S_O$ 可以转移为 $S_D$，如果 TC 不存在，说明这个转移是无条件发生的； 
        - 输出有两种表达方式（Mealy&Moore），分别根据实际情况使用：
            - 如果状态和输出可绑定，可以使用 Moore 型，从节点处引出一条线标注输出；
            - 如果状态和输出不可绑定，可使用 Mealy 型，从 TC 处引出一条线，标注输出条件和输出结果，用 `/` 分隔；如果输出条件和转移方程相同，可以直接用 `/` 将输出写在 TC 后面；
    ...
    ???+ note "详细解释"
        ...
        更系统的来说，状态机模型主要有三要素：**输入条件(Input Conditions)**、**状态转移(Transitions)** 和 **输出行为(Output Actions)**。总体来说，输入条件告诉我们整个状态机要如何变化；状态转移描述了状态机的内部转移行为，即图论上的有向联通关系；输出行为描述动态变化过程中状态机的对外表达。其中，状态转移和输出行为是我们需要关注的状态机的两个维度，输入条件是我们用来做实时判断的外部依据。
