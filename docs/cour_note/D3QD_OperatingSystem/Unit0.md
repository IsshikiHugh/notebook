...
需要注意，操作系统发现中断后并不是二话不说马上去处理中断的，为了保证中断处理完成后仍能继续当前任务，操作系统需要保存当前任务的状态，以便完成中断处理后恢复当前任务的状态。在 [lab1](https://zju-sec.github.io/os23fall-stu/lab1/#rv64_1){target="_blank"} 中，我们会涉及相关内容。
