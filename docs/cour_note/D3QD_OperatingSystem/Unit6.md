...
???+ eg "🌰"
    ...
    === "题面"
        ...
        一个文件系统使用二级索引分配，一个块的大小为 1 KB，索引中的块地址为 4 B，则文件系统能支持的最大空间大小为？

    === "解析"

        这里有两条线，第一条是索引系统所能支持的最大块数量，第二条是块地址能支持的最大地址空间大小，我们应当分别计算，取最小的那个作为答案。

        首先，可以认为二级索引实际上就是个二维数组，一个维度由一个块维护，所以每个维度中有 1 KB / 4 B = 2^8^ 个项，所以一共支持 2 ^16^ 个项。

        其次，由于块地址由 4 B 表示，所以地址空间最多有 2^32^ 个项。

        所以，文件系统支持的最多项数为 2^16^ 项，最大空间为 1 KB * 2^16^ = 64 MB。
