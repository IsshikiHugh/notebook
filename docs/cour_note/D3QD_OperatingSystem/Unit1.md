...
> [虚拟地址空间](./Unit3.md#virtual-address-space){target="_blank"}将会在后面的章节中详细讲解，读者可以考虑访问链接简单瞥一眼留个印象。
...
!!! extra "copy on write"
    ...
    > 本块内容于[虚拟内存](./Unit3.md#回顾-copy-on-write){target="_blank"}相关的内容有关，如果读者想不明白具体实现，可以先了解功能，在之后学习相关内容后我会再次提醒读者回顾该部分内容。
