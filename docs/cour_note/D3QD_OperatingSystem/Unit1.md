...
> [虚拟地址空间](./Unit3-Part1.md#virtual-address-space){target="_blank"}将会在后面的章节中详细讲解，读者可以考虑访问链接简单瞥一眼留个印象。
...
!!! extra "copy on write"
    ...
    > 本块内容于[虚拟内存](./Unit3-Part2.md#回顾-copy-on-write){target="_blank"}相关的内容有关，如果读者想不明白具体实现，可以先了解功能，在之后学习相关内容后我会再次提醒读者回顾该部分内容。
...
- 信号量(semaphores)：
    - 我们将在[进程同步这一章](./Unit2-Part1.md#semaphores){target="_blank"}中重点讲解信号量；
- 共享内存(shared memory)：
...
优先级调度的思路是，每个进程都有一个优先级，当有多个进程处于就绪态时，选择优先级最高的进程先运行。这种算法的优点是，能够保证优先级高的进程优先运行；但是缺点是，如果有一个进程的优先级很低，那么它可能会一直被推迟，从而导致“**饥饿**”现象。
